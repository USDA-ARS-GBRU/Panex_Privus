...
.controls {
  display: grid;
  grid-template-columns: minmax(260px, 1.2fr) minmax(240px, 1fr) minmax(240px, 1fr) minmax(220px, .9fr);
  gap: 12px;
  align-items: stretch;
  margin: 14px 0;
}
...
.export-note { margin: 7px 0 0; color: var(--muted); font-size: .82rem; line-height: 1.35; }
select, input {
  width: 100%;
  border: 1px solid #c9d2cc;
  border-radius: 7px;
...
@media print {
  body { background: #fff; }
  .site-header { padding: 14px 0 10px; background: #fff; border-bottom: 1px solid #d7ddd8; }
  main { padding: 14px 0 0; }
  .summary-grid, .controls, .candidate-panel, .notes { display: none !important; }
  .section { border: 0; box-shadow: none; padding: 0; margin: 0; }
  .section-head p { display: none; }
  .browser-wrap { border: 1px solid #bfc9c3; break-inside: avoid; }
  .browser-detail { grid-template-columns: 1fr; break-inside: avoid; }
  #genomeCanvas { min-height: 0; page-break-inside: avoid; }
  .site-footer { padding: 14px 0 0; }
  ...
  <section class="section">
    ...
    <div class="controls">
      ...
      <div class="control-group">
        ...
        <p class="export-note">Uses the current zoom window and visible tracks.</p>
      </div>
    </div>
    <div class="control-group">
      <label>Tracks</label>
...
function visibleTrackLabels() {
  const labels = [];
  if (state.toggles.density) labels.push("density");
  if (state.toggles.snp) labels.push("PASS SNPs");
  if (state.toggles.indel) labels.push("INDEL/complex");
  if (state.toggles.sv) labels.push("SV-like");
  if (state.toggles.genes) labels.push("genes/exons/CDS");
  if (state.toggles.promoters) labels.push("promoters");
  if (state.toggles.gff && DATA.tracks.length) labels.push("extra GFF3 tracks");
  return labels.join(", ");
}
function downloadCanvasPng() {
  ...
  out.height = Math.round((headerHeight + cssHeight + footerHeight) * scale);
  const outCtx = out.getContext("2d");
  outCtx.scale(scale, scale);
  outCtx.fillStyle = "#ffffff";
  outCtx.fillRect(0, 0, cssWidth, headerHeight + cssHeight + footerHeight);
  outCtx.fillStyle = COLORS.text;
  outCtx.font = "700 20px -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif";
  outCtx.fillText(S.title, 24, 30);
  outCtx.fillStyle = COLORS.muted;
  outCtx.font = "13px -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif";
  outCtx.fillText(`${S.contig}:${fmt(state.start)}-${fmt(state.end)}; visible tracks: ${visibleTrackLabels() || "none"}`, 24, 55);
  outCtx.fillText("Generated with Panex Privus (privy): https://github.com/USDA-ARS-GBRU/Panex_Privus", 24, 76);
  outCtx.strokeStyle = COLORS.line;
  outCtx.beginPath();
  outCtx.moveTo(24, headerHeight - 10);
  outCtx.lineTo(cssWidth - 24, headerHeight - 10);
  outCtx.stroke();
  outCtx.drawImage(canvas, 0, headerHeight, cssWidth, cssHeight);
  const footerY = headerHeight + cssHeight + 24;
  outCtx.strokeStyle = COLORS.line;
  outCtx.beginPath();
  outCtx.moveTo(24, footerY - 13);
  outCtx.lineTo(cssWidth - 24, footerY - 13);
  outCtx.stroke();
  outCtx.fillStyle = COLORS.muted;
  outCtx.font = "12px -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif";
  outCtx.fillText("This static image was exported from a self-contained Panex Privus interactive focus-region report.", 24, footerY + 8);
  outCtx.fillText(`Source interval: ${S.contig}:${fmt(S.start)}-${fmt(S.end)}; current window: ${fmt(state.start)}-${fmt(state.end)}.`, 24, footerY + 29);
  out.toBlob(blob => {
    if (!blob) return;
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    const stem = `${S.contig}_${Math.round(state.start)}_${Math.round(state.end)}`.replace(/[^A-Za-z0-9_.-]+/g, "_");
    link.href = url;
    link.download = `panex_privus_focus_${stem}.png`;
...
function setupControls() {
  ...
  document.getElementById("exportPng").addEventListener("click", downloadCanvasPng);
