...
an existing result directory with the `landscape` plot set:
...
Use `--plot-set landscape` to render heatmaps, local background maps, and
similarity cluster maps from a `privy landscape` output directory:
